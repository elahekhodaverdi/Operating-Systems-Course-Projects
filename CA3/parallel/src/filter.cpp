...
namespace filter
{
    ...
    void kernel(long tid, BMPImage &img, BMPImage &copy, Kernel kernel)
    {
        ...
        int start_row = float(float(height) / NUMBER_OF_THREADS) * tid;
        int end_row = float(float(height) / NUMBER_OF_THREADS) * (tid + 1);
    ...
    {
        ThreadArgs *args = static_cast<ThreadArgs *>(thread_arg);
        args->img.flip(args->tid);
        pthread_exit(NULL);
    }
    ...
    void *blur(void *thread_arg)
    {
        ...
        kernel(args->tid, args->img, args->copy_img, blur_kernel);
        pthread_exit(NULL);
    }
    void *purplehaze(void *thread_arg)
    {
        ThreadArgs *args = static_cast<ThreadArgs *>(thread_arg);
        int height = args->img.get_height();
        int width = args->img.get_width();
        int start_row = float(float(height) / NUMBER_OF_THREADS) * args->tid;
        int end_row = float(float(height) / NUMBER_OF_THREADS) * (args->tid + 1);

        for (int i = start_row; i < end_row; ++i)
        {
            for (int j = 0; j < width; ++j)
            {
                auto &pixel = args->img(i, j);
                float red = 0, green = 0, blue = 0;
    ...
    void *lines(void *thread_arg)
    {
        ...
        int width = args->img.get_width();
        if (args->tid == 0)
            setLine(args->img, width, 0, width / 2, height / 2, 0, 255);
        if (args->tid == 1)
            setLine(args->img, width, 0, width - 1, height - 1, 0, 255);
        if (args->tid == 2)
            setLine(args->img, width, height / 2, width - 1, height - 1, width / 2, 255);
        pthread_exit(NULL);
    }
